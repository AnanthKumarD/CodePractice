...
var scheduleCourse = function(courses) {
    ...
    var maxEndDateTime = 0;
    courses.forEach( function(element, index) {
    	if(element[1] > maxEndDateTime){
    		maxEndDateTime = element[1]
    	}
    });
   	var coursesTobeConsidered = [];
   	sum = 0;
    courses.forEach( function(element, index) {
    	sum = sum + element[0];
    	if(sum <= maxEndDateTime){
    		coursesTobeConsidered.push(element[0])
    	}
    });

    return coursesTobeConsidered.length;
};
